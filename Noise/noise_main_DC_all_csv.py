...
base_directory = r"F:\Personal Data\NbO2\Size dependent study\Line 5\New folder\16 Min"
...
for filenumber in range(start_file_number, end_file_number + 1):
    ...
    dataframe = pd.read_csv(path_csv + os.sep + file_format, delimiter=',',
                                header=None,  skiprows=1, skipfooter=0, nrows=None, low_memory=False, encoding_errors="ignore")
    
    i = dataframe.iloc[:, 1].to_numpy()
   #i = dataframe.iloc[:, 14].to_numpy()
    bias_input = dataframe.iloc[5, 14]
    ...
    psd_file(_write=True) 
    '''Alpha Calculate for unormalized'''
    ...
    # Write header row
    header = ['File number', 'Avg of Fluc', 'Slope', 'Error in slope', 'R Square',
              'Noise at 3.9 mHz', 'Noise at 10 mHz', 'Noise at 100 mHz', 'Noise at 1 Hz',
              'Temp (K)', 'Volt/Curr Supplied (V/A)']
    for col_num, value in enumerate(header, 1):
        sheet.cell(row=1, column=col_num, value=value)
    
    # Write data row
    data = [int(filenumber), iavg, slope, std_err, rsq, foooi, fooi, foi, fi, temperature, bias_input]
